...
final unitsProvider = FutureProvider(
  (ref) => ref.watch(catalogServiceProvider).fetchUnits(),
);

final gamesForUnitProvider =
    FutureProvider.family<List<GameDefinition>, String>((ref, unitKey) {
      return ref.watch(catalogServiceProvider).fetchGames(unitKey: unitKey);
    });

final allGamesProvider = FutureProvider(
  (ref) => ref.watch(catalogServiceProvider).fetchGames(),
);

/// Live "شجرة التقدم" sync — replaces the `sync_tree_growth` socket event.
final studentProgressProvider = StreamProvider.family<StudentProgress?, String>(
  (ref, studentId) {
    return ref
        .watch(realtimeServiceProvider)
        .watchStudentProgress(studentId)
        .map(
